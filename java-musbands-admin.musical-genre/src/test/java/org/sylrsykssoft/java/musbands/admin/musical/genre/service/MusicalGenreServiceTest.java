...
public class MusicalGenreServiceTest {
	...
	@Autowired
	private MusicalGenreAuditService musicalGenreService;

	private MusicalGenreAuditResource domain;

	private Example<MusicalGenreAuditResource> example;
	...
	public void setUp() {
		...
		domain = MusicalGenreAuditResource.musicalGenreAuditResourceBuilder().name(name).description(description)
				.build();
	...
	public void testCreateMusicalGenre() {
		final String name = "Pop punk";
		final MusicalGenreAuditResource musicalGenreSaved = musicalGenreService.create(domain);
	...
	public void testDeleteMusicalGenre() {
		final String name = "Pop punk";
		final Optional<MusicalGenreAuditResource> optResource = musicalGenreService.findByName(name);

		final MusicalGenreAuditResource source = optResource.orElseThrow(IllegalStateException::new);
	...
	public void testFindAllMusicalGenres() {
		final Iterable<MusicalGenreAuditResource> musicalGenres = musicalGenreService.findAll();
	...
	public void testFindOneByExampleMusicalGenre() {
		final Optional<MusicalGenreAuditResource> optResource = musicalGenreService.findByExample(example);
	...
	public void testUpdateMusicalGenre() {
		...
		final MusicalGenreAuditResource resourceGet = optResource.orElseThrow(IllegalStateException::new);

		assertThat("Entity name did not match!", resourceGet.getName(), is(name));

		final MusicalGenreAuditResource resourceUpdated = musicalGenreService
				.update(MusicalGenreAuditResource.musicalGenreAuditResourceBuilder().entityId(resourceGet.getEntityId())
